...
- **目前进展**
  ...
  - 效果上视频风格化效果不好，问题在于视频帧间天空误检漏检不一致问题、以及考虑实时性需要降低图像分辨率导致细节等模糊
  - 速度上采用低分辨率（240$\times$360~360$\times$480）可以达到实时（约15~20FPS），但细节效果上有损耗，高分辨率不能实时（可以做成单帧图像的风格化）
...
1. 手机端要达到实时（13-20FPS左右），图像支持最大尺寸约在360$\times$480~320$\times$640之间。
2. 手机端速度是PC端的$1\over2$ ~ $1\over3$
3. 单个贴天空步骤的时间大约是整个时间的 $1\over3$
4. 当前速度针对大尺度的图有优化空间，例如：检测天空可以用缩小的图不用大图，简化某些操作步骤等；

![1536030732571](style_transfer_2018_08_26.assets/1536030732571.png)

**说明：**

由于分辨率降低，对整体效果有较大影响，下图为几张手机demo的示例图。

+ 不同分辨率的结果对比如下（从上到下图像大小依次是$240\times480$、$360\times640$、$720\times1080$）
+ 大于$360\times640$差别不会特别大，但小于$360\times640$的效果差别比较明显
+ 不同手机的计算能力和速度不同，达到实时所支持的最高分辨率也不一样（例如，iphone7可以360$\times$720，但iPhone6只能360$\times$480）

![1536040139364](style_transfer_2018_08_26.assets/1536040139364.png)

+ 未贴天空

![1536032345246](style_transfer_2018_08_26.assets/1536032345246.png)

+ 贴天空

![1536032315177](style_transfer_2018_08_26.assets/1536032315177.png)

#### 6. 针对视频的问题

+ 天空检测不稳定

+ 为了满足实时性，需要降低图像分辨率，图像细节都被模糊，效果受影响
...
2. 对视频，除了上面单帧图像的问题，还有存在帧与帧之间的不连续性、抖动性等问题。最严重的问题有两个：
   + 天空的影响（在连续帧之间不同）
   + 由于要达到实时性，必须压缩图像尺寸，分辨率降低影响了风格化的细节效果
3. 速度来说，$240\times480$~$360\times640$尺寸下在iphone7上可以达到实时，但有以下问题：
   + 在计算能力更弱的机型上，速度更慢，同样图像尺寸达不到实时
   + 降低图像尺寸可以提高速度，达到实时的目的，但同时影响了风格化的细节效果
   + **目前想法：**
     + 针对视频，采用低分辨率，力求实时
     + 针对单帧图像，采用高分辨率，力求效果
     + 不同机型，支持的最大图像尺寸可以不同
...
+ 视频风格化，在效果和速度上很难取得平衡。高清视频的风格化对计算资源和能力要求特别高，依目前的硬件水平应该很难做到在手机端达到高清视频风格化。即使是传到服务器端进行风格化（就算用到了gpu加速），不但对服务器端压力很大，而且由于上传和接收的都是图像还是大图，这对网络的带宽、速度、稳定性等要求也很高（如果是视频，对服务器端的响应稳定性等要求还是蛮高的）。
